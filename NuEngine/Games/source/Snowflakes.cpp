...
void Snowflakes::Render(nu::console::ConsoleRenderer& renderer)
{
	...
	auto renderStat = [&renderer, x](uint16_t y, std::string_view label, auto time, auto budget, bool shouldInvertColor)
	{
		auto color = time <= budget ? vt::color::ForegroundBrightGreen : vt::color::ForegroundBrightRed;
		if (shouldInvertColor)
		{
			color = color == vt::color::ForegroundBrightGreen ? vt::color::ForegroundBrightRed : vt::color::ForegroundBrightGreen;
		}
